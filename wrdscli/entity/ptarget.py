...
class PriceTarget(WRDSEntity):
  ...
  @staticmethod
  async def from_cusip(cusip, limit=None, remove_check=True):
    if remove_check:
      cusip = cusip[:-1]
    return await super(PriceTarget, PriceTarget).from_attr(PriceTarget, 'cusip', cusip, exact=True, limit=limit)