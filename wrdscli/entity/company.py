...
class Company(WRDSEntity):
  ...
  @staticmethod
  async def from_name(name):
    '''
    Return 0 or more companies
    '''
    return await super(Company, Company).from_attr(Company, 'conm', name.upper())

  @staticmethod
  async def from_gvkey(gvkey):
    res = await super(Company, Company).from_attr(Company, 'gvkey', gvkey, exact=True)
    if len(res) > 1:
